...
/**
 * Base reporting listener for Extent reports, logging, screenshots, and cleanup.
 * No environment or browser initialization occurs here — handled by glue-level hook.
 */
public class ListenerPlugin implements ConcurrentEventListener, io.cucumber.plugin.Plugin {
    ...
    public void onTestCaseFinished(TestCaseFinished event) {
        ...
        String reason = event.getResult().getError() != null
        ...
        switch (status) {
            ...
            case FAILED -> {
                ...
                if (isXFail) testReport.xfail(testName, reason, getFeatureName());
                else testReport.fail(testName, reason, getFeatureName());
            }
    ...
    public static final EventHandler<TestStepFinished> stepFinishedHandler = event -> {
        if (event.getTestStep() instanceof PickleStepTestStep) {
            ...
            if (current != null) current.info("✅ Step Finished: " + getStepKeyword() + getStepName());
    ...
    private static void scrollAndShot() {
        ...
        do {
            ...
            ExtentCucumberAdapter.getCurrentStep().info(
                MediaEntityBuilder.createScreenCaptureFromBase64String(driver.getScreenshotAs(OutputType.BASE64)).build()
            );
